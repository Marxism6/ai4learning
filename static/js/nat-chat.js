...
(function (N) {
  ...
  N.sendMessage = async function (message) {
    ...
    try {
      var body = { username: N.state.username, message: message, block_slug: N.state.blockSlug, history: N.state.history.slice(0, -1), memory_enabled: N.state.memoryEnabled, lang: N.state.lang, study_mode: N.state.studyMode }; if (mem) body.memory_summary = mem;
