...
(function (N) {
  ...
  N.renderHistoryList = function () {
    ...
    var key = 'history-' + N.state.username; var list = N.lsGetJSON(key, []);
    if (!list.length) { N.els.historyList.innerHTML = '<p class="body-sm" style="color:var(--ink-faint);padding:12px;">' + N.t('emptyHistory') + '</p>'; return; }
    N.els.historyList.innerHTML = ''; var sorted = list.slice().reverse();
  ...
  N.viewHistory = function (entry) {
    N.closeHistory();
    N.state.history = entry.history.slice();
    N.state.blockSlug = entry.blockSlug || null;
  ...
  N.bindHistoryEvents = function () {
    ...
    N.els.clearHistoryBtn.addEventListener('click', function () {
      ...
      N.lsRemove('history-' + N.state.username); N.renderHistoryList();
    });
  };
